...
export class IntegratedEmailMonitoringService {
  ...
  private async initializeGraphClient() {
    try {
      ...
      const { TokenCredentialAuthenticationProvider } = require('@azure/msal-node');
  ...
  async startMonitoring(): Promise<void> {
    ...
    this.isRunning = true;
    const pollInterval = this.config.get('email.microsoftGraph.pollIntervalMinutes', 1);
  ...
  getStatus() {
    return {
      ...
      config: {
        ...
        pollInterval: this.config.get('email.microsoftGraph.pollIntervalMinutes', 1),
        enabled: this.config.get('email.microsoftGraph.enabled', true)
      }
    };
  }
